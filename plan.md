...
|------|--------|-------------|--------------|
| **%** (수익률) | **2자리** | `{:.2f}` | 금리 (%), 현재(%), 최종호가(%), 장외거래(%) |
| **bp** (변화량) | **1자리** | `{:.1f}` | 1D, 1W, MTD, MoM, YTD, YoY, 스프레드(bp) |
...
채권 탭:
  ├─ [글로벌 국채 금리]        ✅ 구현 완료
  │     - 주요국 금리 요약 테이블 (1D/1W/MTD/MoM/YTD/YoY bp, 소수점 1자리)
  │     - 국가별 Yield Curve 차트 (현재/1W/1M)
  │     - KR 열: bond_summary에서 KTB_nY → KR_nY 변환 후 병합
  │
  ├─ [국내 채권 금리]          ✅ 구현 완료
  │     - 전종목 금리 요약 테이블 (금리 % 소수점 2자리, 변화 bp 소수점 1자리)
  │     - 국고채 Yield Curve 차트
